...
for i in tracks.index:
    playlist.append(
        (tracks[headers[0]][i], tracks[headers[1]][i], tracks[headers[2]][i])
    )
...
def media():
    ...
    return render_template(
        "media.html", tracks=playlist, current_track=current_track
    )  # , thread=thread)
...
def power_strip():
    ...
    if request.method == "POST":
        ...
        if name_form != None and ip_form != None:
            if re.match(
                "^((25[0-5]|(2[0-4]|1[0-9]|[1-9]|)[0-9])(\.(?!$)|$)){4}$", ip_form
            ):
                ...
                if not has_name and not has_ip:
                    power_nodes.append(
                        PowerNode(name_form, f"http://{ip_form}/sensors")
                    )
            else:
                print("Not valid ip")

    return render_template(
        "power_strip.html", power_nodes=power_nodes, enumerate=enumerate
    )
    ...
    if request.method == "POST":
        ...
        if name_form != None and ip_form != None:
            if re.match(
                ...
                if not has_name and not has_ip:
                    sensor_nodes.append(
                        SensorNode(name_form, f"http://{ip_form}/sensors")
                    )
            else:
                print("Not valid ip")
    ...
    for node in sensor_nodes:
        if node.timer_enabled:
            new_job = scheduler.add_job(
                func=node.update_power_node, trigger="interval", seconds=60
            )
            scheduler_jobs.append(new_job)
